...
namespace InvalidSiteUserDetails
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                foreach (var userEle in userEles)
                {
                    ...
                    {
                        bool bln = DoesUserExist(userName);
                        if (!bln)
                        {
        ...
        private static bool DoesUserExist(string strUser)
        {
            PeopleProxy.People pplProxy = null;
            bool bUsrResolved = false;
            PeopleProxy.PrincipalInfo[] pInfoArr = null;
            try
            {
                pplProxy = new PeopleProxy.People();
                pplProxy.Url = SITE_URL + "/_vti_bin/people.asmx";
                pplProxy.Credentials = new NetworkCredential(USER_NAME, PASSWORD, DOMAIN);
                pInfoArr = pplProxy.ResolvePrincipals(new string[] { strUser }, PeopleProxy.SPPrincipalType.User, false);
                if ((pInfoArr != null) && (pInfoArr.Length > 0))
                {
                    bUsrResolved = pInfoArr[0].IsResolved;
                }
                return bUsrResolved;
            }
            catch
